...
var HistoricalForecast = function(options){
	...
	this.opts = {
		...
		loc: 'chicago',
		updateHourUTC: 12 // make noon, 12pm exactly, be the precise 'time' for a given day
	};
 ...
 */
HistoricalForecast.prototype.run = function() {
	this.fetchAndUpdateForecasts();
	//this.updateActuals();
}

/**
 * updateForecasts: fetches new data from wunderground.com and updates apiServer
 */
HistoricalForecast.prototype.fetchAndUpdateForecasts = function() {
	var me = this;
	var url = this.opts.apiServer + '/forecast?loc=chicago.updating';
	// $.getJSON('http://api.wunderground.com/api/121b8928ea8cca08/forecast10day/q/IL/Chicago.json?callback=?')

	this.getJson( url, function(err, apiJsonData) {
		if (err) return;
		
		url = 'http://api.wunderground.com/api/121b8928ea8cca08/forecast10day/q/IL/Chicago.json';
		me.getJson( url, function(err, wuJsonData) {
			if (err) return;
			me.updateForecasts(apiJsonData, wuJsonData);
		});
	});
}

/**
 * updateForecasts: parses json data from api server and wuderground, then updates api server
 * @param {Object} apiJsonData fetched from api server
 * @param {Object} wuJsonData fetched from api.wunderground.com
 */
HistoricalForecast.prototype.updateForecasts = function(apiJsonData, wuJsonData) {
	var ids = {};
	var ids = {};
	var url, ii;
	var epochSecs;
	...
	for (ii=0; ii < apiJsonData.length; ii++) {
		...
		if (epochSecs * 1000 < nowMs) {
			url = this.opts.apiServer + '/forecast/update/'+ ids[epochSecs].id +'?loc=chicago';
