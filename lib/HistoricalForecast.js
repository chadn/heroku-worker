...
HistoricalForecast.prototype.run = function() {
	...
	this.asyncGetJson('wuActuals', 'http://api.wunderground.com/api/121b8928ea8cca08/conditions/q/IL/Chicago.json'); // TODO wuActuals
	var me = this;
	this.whenDone(['api', 'wuForecast', 'wuActuals'], function() { me.updateApiServer() });
}
...
HistoricalForecast.prototype.updateApiServer = function() {
	...
	var url, ii;
	var epochSecs;
	var days = this.getJ.wuForecast.json.forecast.simpleforecast.forecastday;
	
	for (ii=0; ii < this.json.api.length; ii++) {
		// create a hash of existing document Ids with 'day' as key
		epochSecs = this.json.api[ii]['day'];
		ids[epochSecs] = this.json.api[ii];
